...
@pytest.fixture
def kinematic_model():
    alpha = np.array([0, math.pi / 2, math.pi, math.pi * 3 / 2]).reshape(-1, 1)
    l = np.array([[1.0]] * 4)
    b = np.array([[0.0]] * 4)
    r = np.array([[0.1]] * 4)
    k_beta = 1.0
...
def test_actuators_motion_reverse(kinematic_model):
    # lmda = np.array([[0], [0], [1]])
    # lmda_dot = np.array([[0], [0], [1]])
    lmda = np.array([[0.99978376], [-0.0], [0.02079483]])
    lmda_dot = np.array([[0.02162124], [0.0], [-1.03951656]])
    lmda_2dot = np.array([0, 0, -51.98706951]).reshape(-1, 1)
    mu = 126.4
    mu_dot = 1.0
    beta_prime, _, _, _ = kinematic_model.compute_actuators_motion(
        lmda, lmda_dot, lmda_2dot, mu, mu_dot
    )
    # Flip lambda an mu
    lmda = -lmda
    ...
    mu = -mu
    mu_dot = -mu_dot
    beta_prime_flip, _, _, _ = kinematic_model.compute_actuators_motion(
        lmda, lmda_dot, lmda_2dot, mu, mu_dot
    )
    assert np.allclose(beta_prime, beta_prime_flip)
...
def test_singularity_on_wheel(kinematic_model):
    ...
    assert np.isclose(phi_dot[0,0], 0, atol=1e-2)
...
def test_s_perp(kinematic_model):
    ...
    assert np.allclose(s_lmda[0], expected_s1)
...
def test_estimate_mu_manual(kinematic_model):
    ...
    assert np.isclose(
        mu, expected * kinematic_model.r[0,0], atol=0.01
    ...
    mu = kinematic_model.estimate_mu(phi_dot, lmda)
    assert np.isclose(mu, expected * kinematic_model.r[0,0], atol=0.01), (
        "\nCalculated mu: %f,\nexpected: %f\nlambda: %s"
...
def test_compute_odometry(kinematic_model):
    ...
    assert np.allclose(
        xi, np.array([x_dot * dt, y_dot * dt, 0.0]).reshape(-1, 1), atol=1e-2
    )
...
def test_compute_chassis_motion_no_motion(kinematic_model):
    ...
    dlmda, d2lmda, dmu = kinematic_model.compute_chassis_motion(
        lmda_d, lmda_e, mu_d, mu_e, k_b, phi_dot_bounds, k_lmda, k_mu
    )
    assert np.allclose(dlmda, np.array([[0]] * 3))
    assert np.allclose(d2lmda, np.array([[0]] * 3))
    assert np.allclose(dmu, 0)
