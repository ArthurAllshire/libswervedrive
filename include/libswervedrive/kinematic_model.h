...
public:
  ...
  std::pair<Nu, Lambda> compute_chassis_motion(Lambda lambda_desired, double mu_desired, Lambda lamda_estimated,
                                               double mu_estimated, double k_backtrack, double k_lambda, double k_mu);
  double compute_mu(Lambda, double phi_dot);
  Motion compute_actuator_motion(Lambda lambda, Lambda lambda_dot, Lambda lambda_2dot, double mu, double mu_dot);
