...
void assert_scaling_bounds(ModuleMotion module_motion, Bounds beta_dot_bounds, Bounds beta_2dot_bounds,
                           Bounds phi_2dot_bounds)
{
  ...
  auto bounds = scaler.computeScalingBounds(motion);

  if (!ignore_beta)
  {
    // check we satisfy equation 36a
    EXPECT_TRUE(bounds.first.lower >= beta_dot_bounds[0][lower_beta] / module_motion.beta_dot);
    EXPECT_TRUE(bounds.first.upper <= beta_dot_bounds[0][upper_beta] / module_motion.beta_dot);

    // check that we satisfy equation 36b
    double ds_upper_sq = std::pow(bounds.first.upper, 2);
    EXPECT_TRUE(bounds.second.lower >=
                (beta_2dot_bounds[0][lower_beta] - module_motion.beta_2dot * ds_upper_sq) / module_motion.beta_dot);
    EXPECT_TRUE(bounds.second.upper <=
                (beta_2dot_bounds[0][upper_beta] - module_motion.beta_2dot * ds_upper_sq) / module_motion.beta_dot);
  }
  if (!ignore_phi)
  {
    EXPECT_TRUE(bounds.first.upper >= phi_2dot_bounds[0][lower_phi] / module_motion.phi_2dot);
    EXPECT_TRUE(bounds.first.lower <= phi_2dot_bounds[0][upper_phi] / module_motion.phi_2dot);
  }

  // then compute scaling parameters
  ScalingParameters scale_params = scaler.computeScalingParameters(bounds.first, bounds.second);

  // then use this to scale the motion
  Motion scaled_motion = scaler.scaleMotion(motion, scale_params);
